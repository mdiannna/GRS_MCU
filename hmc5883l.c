...
void hmc5883l_getdata(double *mx, double *my, double *mz) {
	...
	float mxt = mxraw - biasX_s[1];
	float myt = myraw - biasY_s[1];
	float mzt = mzraw - biasZ_s[1];
	*mx = gainX1_s[1] * mxt + gainX2_s[1] * myt +  gainX3_s[1] * mzt;
	*my =  gainY1_s[1] * mxt + gainY2_s[1] * myt + gainY3_s[1] * mzt;
	*mz = gainZ1_s[1] * mxt + gainZ2_s[1] * myt + gainZ3_s[1] * mzt;
	#else
